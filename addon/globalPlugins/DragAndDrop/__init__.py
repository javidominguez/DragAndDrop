...
#Copyright (C) 2016-2021 Javi Dominguez <fjavids@gmail.com>
	...
	def script_mouseCursorInfo(self, gesture):
		...
		if scriptHandler.getLastScriptRepeatCount() == 1:
			if not self.objectToDrag:
				...
				return
			REASON = controlTypes.REASON_MOUSE if hasattr(controlTypes, "REASON_MOUSE") else controlTypes.OutputReason.MOUSE
			speech.speakObject(self.objectToDrag, reason=REASON)
		...
		REASON = controlTypes.REASON_MOUSE if hasattr(controlTypes, "REASON_MOUSE") else controlTypes.OutputReason.MOUSE
		speech.speakObject(mouse, reason=REASON)
